...
CODING_OVERRIDE = [
    "implement", "write code", "time complexity", "output of code", "debug",
    "leetcode", "binary search", "dfs", "bfs", "linked list", "array", "write a function",
    "given an array", "given a string"
]

SYSTEM_DESIGN_OVERRIDE = [
    "architecture", "architect", "design a", "scalable", "distributed",
    "pipeline", "microservices", "high availability", "fault tolerance",
    "throughput", "latency"
]
...
def build_category_embeddings(model):
    ...
    for cat_name, data in CATEGORIES.items():
        ...
        prototype_embedding = model.encode([data["prototype"]], normalize_embeddings=True)[0]
        cat_embs[cat_name] = {
            "anchors": anchor_embeddings,
            "prototype": prototype_embedding
        }
        
    # Build Stage 1 Super-Categories
    # We combine the anchors arrays, and average the prototypes
    super_embs = {
        "coding": {
            "anchors": cat_embs["coding"]["anchors"],
            "prototype": cat_embs["coding"]["prototype"]
        },
        "engineering": {
            "anchors": np.vstack([cat_embs["technical"]["anchors"], cat_embs["system_design"]["anchors"]]),
            "prototype": (cat_embs["technical"]["prototype"] + cat_embs["system_design"]["prototype"]) / 2.0
        },
        "behavioral": {
            "anchors": np.vstack([cat_embs["hiring_manager"]["anchors"], cat_embs["recruiter"]["anchors"]]),
            "prototype": (cat_embs["hiring_manager"]["prototype"] + cat_embs["recruiter"]["prototype"]) / 2.0
        }
    }
    
    # Normalize the averaged super prototypes
    for s_cat in super_embs:
        norm = np.linalg.norm(super_embs[s_cat]["prototype"])
        if norm > 0:
            super_embs[s_cat]["prototype"] = super_embs[s_cat]["prototype"] / norm
            
    return cat_embs, super_embs
...
def score_category(q_emb, cat_data):
    anchors_emb = cat_data["anchors"]
    proto_emb = cat_data["prototype"]
    
    # Dot product for cosine similarity (since vectors are normalized)
    anchor_sims = np.dot(anchors_emb, q_emb)
    max_anchor_sim = np.max(anchor_sims)
    
    proto_sim = np.dot(proto_emb, q_emb)
    
    return (0.8 * max_anchor_sim) + (0.2 * proto_sim)

def process_file(json_file_path):
    print("Loading SentenceTransformer model...")
    model = SentenceTransformer('all-MiniLM-L6-v2')
    
    cat_embs, super_embs = build_category_embeddings(model)
    ...
    for row in data:
        ...
        for line in lines:
            ...
            for frag in fragments:
                ...
                # STAGE 0: Rule-based Override
                q_lower = frag.lower()
                if any(k in q_lower for k in CODING_OVERRIDE):
                    buckets["coding"].append([frag, "coding", 1.0, "high_override"])
                    continue
    ...
    print(f"Extracted {len(extracted_questions)} non-override questions. Encoding and classifying via Hierarchical pipeline...")
    ...
    for i in range(0, len(extracted_questions), batch_size):
        ...
        for j, question in enumerate(batch):
            ...
            # STAGE 1: Super Categories
            super_scores = []
            for s_cat in ["coding", "engineering", "behavioral"]:
                sc_score = score_category(q_emb, super_embs[s_cat])
                super_scores.append((float(sc_score), s_cat))
                
            super_scores.sort(key=lambda x: x[0], reverse=True)
            best_sc_score, best_sc = super_scores[0]
            second_sc_score, _ = super_scores[1]
            
            # Global junk threshold
            if best_sc_score < SIMILARITY_THRESHOLD:
                buckets["unclassified"].append([question, "unclassified", round(best_sc_score, 4), "low"])
                continue
                
            # Stage 1 Ambiguity Check
            if (best_sc_score - second_sc_score) < AMBIGUITY_THRESHOLD:
                buckets["ambiguous"].append([question, f"ambiguous_stage1_{best_sc}", round(best_sc_score, 4), "ambiguous"])
                continue
                
            # STAGE 2: Sub-Categories
            if best_sc == "coding":
                buckets["coding"].append([question, "coding", round(best_sc_score, 4), get_confidence(best_sc_score)])
                
            elif best_sc == "engineering":
                sub_scores = []
                for sub_cat in ["system_design", "technical"]:
                    s_score = score_category(q_emb, cat_embs[sub_cat])
                    sub_scores.append((float(s_score), sub_cat))
                    
                sub_scores.sort(key=lambda x: x[0], reverse=True)
                best_sub_score, best_sub = sub_scores[0]
                second_sub_score, _ = sub_scores[1]
                
                if (best_sub_score - second_sub_score) < AMBIGUITY_THRESHOLD:
                    buckets["ambiguous"].append([question, "ambiguous_engineering", round(best_sub_score, 4), "ambiguous"])
                else:
                    buckets[best_sub].append([question, best_sub, round(best_sub_score, 4), get_confidence(best_sub_score)])
                    
            elif best_sc == "behavioral":
                sub_scores = []
                for sub_cat in ["hiring_manager", "recruiter"]:
                    s_score = score_category(q_emb, cat_embs[sub_cat])
                    sub_scores.append((float(s_score), sub_cat))
                ...
                if (best_sub_score - second_sub_score) < AMBIGUITY_THRESHOLD:
                    buckets["ambiguous"].append([question, "ambiguous_behavioral", round(best_sub_score, 4), "ambiguous"])
                else:
                    buckets[best_sub].append([question, best_sub, round(best_sub_score, 4), get_confidence(best_sub_score)])
    ...
    def write_csv(filename, bucket_key):
        ...
        print(f"Created {filepath} with {len(items)} questions.")

    write_csv('hiring_manager.csv', 'hiring_manager')
    write_csv('recruiter.csv', 'recruiter')
    write_csv('technical_questions.csv', 'technical')
    write_csv('coding_questions.csv', 'coding')
    write_csv('system_design_questions.csv', 'system_design')
    ...
    if all_items:
        filepath = os.path.join(OUTPUT_DIR, 'classified_master_list.csv')
        with open(filepath, 'w', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['question_text', 'category', 'score', 'confidence'])
            for item in all_items:
                writer.writerow(item)
        print(f"Created {filepath} with all {len(all_items)} questions.")
